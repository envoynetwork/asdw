...
contract("Test adding tiers and tokens", function(accounts) {
    ...
    it("Test adding tiers and tokens", async () => {
        ...
        await truffleAssert.reverts(dinowarriors.addTier(testToken1, tokenPrice, 2, dropWave1, web3.utils.asciiToHex(''), {from: minterAddress}),
            "caller is not the owner");
        await truffleAssert.reverts(dinowarriors.addToken(testToken1, tokenAmount, testToken1, 0, {from: minterAddress}),
            "caller is not the owner");

        await dinowarriors.addTier(testToken1, tokenPrice, 2, dropWave1, web3.utils.asciiToHex(''), {from: ownerAddress});
        ...
        assert.equal(web3.utils.hexToAscii((await dinowarriors.tokenInfo.call(testToken1)).tier).replace(/\0/g, ''), web3.utils.hexToAscii(testToken1),
            'Tier was not set correctly')
        assert.equal((await dinowarriors.tokenInfo.call(testToken1)).id, (await dinowarriors.currentTokenId.call()) -1,
        ...
        await truffleAssert.reverts(dinowarriors.addTier(testToken1, tokenPrice, 2, dropWave1, web3.utils.asciiToHex(''), {from: ownerAddress}),
            "Tier already exists")
        ...
        await dinowarriors.addTier(testToken2, tokenPrice, 1, dropWave2, web3.utils.asciiToHex(''), {from: ownerAddress});
        ...
        await truffleAssert.reverts(dinowarriors.addTier(testToken3, tokenPrice, 2, dropWave1, web3.utils.asciiToHex(''), {from: ownerAddress}),
